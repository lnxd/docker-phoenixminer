...
install_new_amd_driver() {
    ...
    apt-get update
    apt-get install -y initramfs-tools &>/dev/null
    apt-get install ./amdgpu-install-$AMD_DRIVER.deb
    apt-get --fix-broken install -y &>/dev/null
    cd /home/docker/
    rm -rf /tmp/opencl-driver-amd
    echo "---AMD Driver installation finished---"
...
if [[ "${INSTALLED_DRIVERV}" != "${DRIVERV:-20.20}" ]]; then
    ...
    21.40.1)
        uninstall_amd_driver
        install_new_amd_driver "https://repo.radeon.com/amdgpu-install/21.40.1/ubuntu/focal" "amdgpu-install_21.40.1.40501-1_all.deb"
        sleep infinity
        ;;
    esac
